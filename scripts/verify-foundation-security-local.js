...
assert.match(posAppFileRoute, /verifyDownloadToken/);
assert.match(posAppFileRoute, /application\/vnd\.android\.package-archive/);
assert.match(posAppFileRoute, /S-and-R-Tap-to-Pay-TEST\.apk|manifest\.filename/);
assert.doesNotMatch(posAppFileRoute, /auth\.requireActiveAdmin/);
assert.doesNotMatch(posAppFileRoute, /sk_live_|sk_test_|whsec_|SERVICE_ROLE/);
